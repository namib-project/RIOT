...
from scapy.all import DUID_LL, DHCP6OptIAPrefix, hexdump, DHCP6OptIA_NA, DHCP6OptIAAddress
...
def testfunc(child):
    ...
    hexdump(pkt[DHCP6OptClientId].duid[DUID_LL].lladdr)
    print(upstream_hwaddrs)
    ...
    address = "b8cb:32d0:2013:ef2a:c6e4:508d:452c:e21f"
    ia_na = DHCP6OptIA_NA(T1=12000, T2=13000, iaid=pkt[DHCP6OptIA_NA].iaid,
                          ianaopts=[
                              DHCP6OptIAAddress(preflft=14000, validlft=15000,
                                                addr=address)])
    ...
    sendp(build_reply_headers(pkt) / DHCP6_Advertise(trid=trid) /
          ia_pd / ia_na / cli_id / srv_id, iface=iface, verbose=False)
    # wait for request
